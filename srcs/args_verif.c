...
/*   Updated: 2024/08/29 23:20:13 by gmarquis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_atoi_safe(char *str)
{
	...
	{
		if (str[i] == '-')
			ft_exit(2, LERR_ARGS_H0, CERR_ARGS_H0);
		i++;
	}
	while (str[i])
	{
...
{
	if(flag == 1)
		ft_exit(2, LERR_ARGS_H2, CERR_ARGS_H1);
	else if(flag == 2)
		ft_exit(2, LERR_ARGS_H3, CERR_ARGS_H1);
}
...
t_sympos	*ft_args_verif_and_make_sympos(int argc, char ** argv)
{
	...
	if (argc < 5 || argc > 6)
		ft_exit(2, LERR_ARGS_H1, CERR_ARGS_H1);
