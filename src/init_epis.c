...
static void	ph_init_epis_kinesis(t_sympos *sympos)
{
	...
	*(sympos->epis->kine->id_dead) = 0;
	...
	pthread_mutex_lock(&sympos->epis->mtx->mtx_sy_states);
	sympos->epis->kine->sy_states = ph_init_malloc_mutex(sympos,
			&sympos->epis->mtx->mtx_sy_states, 1, sizeof(int));
	*(sympos->epis->kine->sy_states) = SETING;
	pthread_mutex_unlock(&sympos->epis->mtx->mtx_sy_states);
